	...
	const AddANewComment = async () => {
		try {
			...
			const result = await axios.post(
				`${process.env.EXPO_PUBLIC_BACKENDURL}/prediction`,
				{
					sentence: commentInput,
				}
			);
			const newCommentInfo = {
				commentDescription: Object.values(result.data).includes(true)
					? "*Hidden due to negativity*"
					: commentInput,
	...
	const renderComment = (comment, idx) => {
		return (
			<View key={idx} style={styles.commentContainer}>
				<Image
					source={require("../../assets/1176433.png")}
		...
		return (
			...
			>
				...
				<View style={styles.actions}>
					<TouchableOpacity
						onPress={newLikeOrRemoveLike}
						style={[styles.likeDislikeCommentBtn]}
					>
						...
						<Text style={styles.likeDislikeCount}>
							({likes.length})
						</Text>
					</TouchableOpacity>
					<TouchableOpacity
						onPress={newDislikeOrRemoveDislike}
						style={[styles.likeDislikeCommentBtn]}
					>
